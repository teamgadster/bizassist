// BizAssist_mobile
// path: app/(app)/(tabs)/settings/units/add.tsx
//
...
import { clearUnitSelectionParams, resolveReturnTo, SETTINGS_UNITS_ROUTE } from "@/modules/units/units.navigation";
...
export default function UnitsAddCategoryScreen() {
	...
	const onExit = useCallback(() => {
		...
		router.replace({
			...
			params: {
				...
				...(inbound.selectedUnitId ? { [UNIT_SELECTED_ID_KEY]: inbound.selectedUnitId } : {}), // Keep this line
			} as any,
		});
	...
	const onNext = useCallback(async () => {
		...
		const nextCategory = category; // Simplified next category
		const nextPrecision = nextCategory === "COUNT" ? DEFAULT_COUNT_PRECISION : DEFAULT_PRECISION; // Simplified precision
		...
		await withBusy("Saving unit...", async () => {
			...
			router.replace({
				...
				params: {
					...
					[DRAFT_ID_KEY]: draftId || undefined,
					[UNIT_CONTEXT_PRODUCT_TYPE_KEY]: productType,
					[UNIT_CREATE_CATEGORY_KEY]: nextCategory, // Keep this line
					// Removed service group from params
