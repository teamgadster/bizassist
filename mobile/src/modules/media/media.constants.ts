// path: src/modules/media/media.constants.ts (MOBILE)
/**
 * Mobile media constraints (must mirror API).
 *
 * Governance:
 * - Keep this file import-free to avoid alias/barrel cycles.
 */
export const MEDIA_ALLOWED_EXT = ["jpg", "jpeg", "png", "webp"] as const;
export type MediaExt = (typeof MEDIA_ALLOWED_EXT)[number];

export const MEDIA_ALLOWED_MIME_TYPES = ["image/jpeg", "image/png", "image/webp"] as const;
export type MediaMimeType = (typeof MEDIA_ALLOWED_MIME_TYPES)[number];

export const EXT_TO_MIME: Record<MediaExt, MediaMimeType> = {
	jpg: "image/jpeg",
	jpeg: "image/jpeg",
	png: "image/png",
	webp: "image/webp",
};

export const MIME_TO_EXT: Record<MediaMimeType, MediaExt> = {
	"image/jpeg": "jpg",
	"image/png": "png",
	"image/webp": "webp",
};


// Product image taxonomy (must mirror API)
export const PRODUCT_IMAGE_KINDS = [
	"PRIMARY_POS_TILE",
	"SECONDARY_GALLERY",
	"VARIANT_SWATCH",
	"LIFESTYLE",
] as const;
export type ProductImageKind = (typeof PRODUCT_IMAGE_KINDS)[number];

// Baseline IQA for POS tile images
export const POS_TILE_MIN_SIZE_PX = 1200;

// Must mirror API guardrail (anti-abuse / operational safety limit).
export const MAX_PRODUCT_IMAGES = 10;
