// BizAssist_mobile
// path: app/(app)/(tabs)/inventory/products/[id]/restore.tsx
//
...
function extractApiErrorMessage(err: unknown): string {
	...
	const msg = data?.message ?? data?.error?.message ?? (err as any)?.message ?? "Failed to restore item.";
	return String(msg);
}

export default function InventoryProductRestoreScreen({ routeScope = "inventory" }: { routeScope?: InventoryRouteScope }) {
	...
	const canRestore = !!product && !product.isActive;
				...
				<View style={styles.screen}>
					<BAISurface style={[styles.card, { borderColor }]} padded bordered>
						<BAIText variant='caption' muted>
							Restored items return to active inventory lists.
						</BAIText>
