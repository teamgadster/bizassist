// BizAssist_mobile
// path: src/modules/inventory/services/screens/InventoryServiceArchiveScreen.tsx
//
...
function extractApiErrorMessage(err: unknown): string {
	...
	const msg = data?.message ?? data?.error?.message ?? (err as any)?.message ?? "Failed to archive service.";
	return String(msg);
}
...
export default function InventoryServiceArchiveScreen({ routeScope = "inventory" }: { routeScope?: InventoryRouteScope }) {
	...
	const canArchive = !!product && product.isActive && isService;
				...
				<View style={styles.screen}>
					<BAISurface style={[styles.card, { borderColor }]} padded bordered>
						<BAIText variant='caption' muted>
							Archived services stay in historical records and are removed from active lists and POS.
						</BAIText>
